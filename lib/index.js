...
import { Image, TextInput, TouchableWithoutFeedback, View, TouchableOpacity, Text } from "react-native";
...
export default class PhoneInput extends Component {
  ...
  render() {
    ...
    const countryDialCodeLength = countryData.dialCode.length+1;
    const clearButtonVisible = formattedNumber.length > countryDialCodeLength && !disabled;
    ...
    return (
      <View style={[styles.container, this.props.style]}>
        ...
        <View style={{ flex: 1, marginLeft: this.props.offset || 10, paddingRight: 10 }}>
          <TextComponent
            ref={ref => {
              this.inputPhone = ref;
            }}
          ...
          {this.props.toggledText && (
            <TouchableOpacity
              style={{ right: clearButtonVisible ? 40 : 10, position: "absolute" }}
              onPress={() => this.props.changeToggleText(this.props.toggledText)}
            >
              <Text style={{ color: "#bbaa64" }} maxFontSizeMultiplier={1}>{this.props.toggledText}</Text>
            </TouchableOpacity>
          )}

          {clearButtonVisible && (
            <View style={{position: "absolute", right: 17, top: 2}}>
              <TouchableOpacity
                style={styles.clearBtnContainer}
                onPress={() => {
                  this.setState({
                    formattedNumber:
                      this.state.formattedNumber.slice(0,
                        countryData.dialCode.length+1)
                  }, () => this.props.onChangePhoneNumber(null))
                }}
              >
                <Image
                  style={{ width: 15, height: 15 }}
                  source={require("./clearBtn.png")}
                  resizeMode="contain"
                />
              </TouchableOpacity>
            </View>
        ...
        <CountryPicker
          ...
          buttonsContainerStyle={this.props.buttonsContainerStyle}
